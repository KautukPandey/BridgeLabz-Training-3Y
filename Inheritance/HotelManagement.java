...
class Hotel {
    ...
    public void mostbookedroom() {
        ...
        List<Integer> bestRooms = new ArrayList<>();
        for (Map.Entry<Integer, Integer> e : count.entrySet()) {
            if (e.getValue() == max) {
                bestRooms.add(e.getKey());
            }
        }

        for (int roomId : bestRooms) {
            Room r = findroom(roomId);
            System.out.println("Most booked room: " + r + " -> " + max + " bookings");
        }

        System.out.println("Total rooms booked in last 6 months: " + totalBookings);
    }
